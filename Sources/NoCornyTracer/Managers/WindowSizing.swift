...
///
/// ЧОГО ЦЕЙ ДОСЛІД НЕ ДОВОДИТЬ (ревʼю 27.08, чесна межа): `async` мутацію не
/// скасовує, лише зсуває на оберт, тож «риси вікна непричетні» з нього НЕ
/// випливає — випливає тільки «відкладення не лікує». Чому та сама бульбашка
/// не падала до 4.5.0, не пояснив ніхто: камерний код у тому релізі не
/// змінювався, старіших звітів із цією сигнатурою немає. Пін лікує незалежно
/// від відповіді, бо рубає петлю у ТОЧЦІ ВХОДУ (SwiftUI просто не пише рамку),
/// а не збиває навантаження під стелю AppKit. Але якщо constraint-runaway
/// колись вилізе на ІНШОМУ вікні — починати треба з мутації `styleMask` у морфі.
enum WindowSizing {
