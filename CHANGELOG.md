...
- **Speaker separation (paid plans).** Transcripts can be split by who was talking, marking each part with a speaker on the recording page and in the subtitles. It works far better with system audio recording on, because the other party then arrives on a track of their own instead of leaking through your microphone. If the count comes out wrong you can correct it afterwards from the Recordings list and Tracer will work the speakers out again, without re-transcribing anything. Paid plans are not on sale yet, so for now this shows as locked.
...
### Changed
- Recordings keep a small copy of their audio (roughly 15 MB per hour) so speakers can be worked out again later without the original file. It is capped and cleaned up automatically. On plans that include speaker separation it is also mirrored to your own Dropbox next to the recording, so it survives this Mac.
